...
prior_1 = train_class_1.shape[0]/train_data_size
prior_2 = train_class_2.shape[0]/train_data_size
prior_3 = train_class_3.shape[0]/train_data_size

print('prior 1: ',prior_1)
print('prior 2: ',prior_2)
print('prior 3: ',prior_3)
...
normalization_factor_1 = prior_1/(2*3.14*np.linalg.det(cov_1)**0.5)
normalization_factor_2 = prior_2/(2*3.14*np.linalg.det(cov_2)**0.5)
normalization_factor_3 = prior_3/(2*3.14*np.linalg.det(cov_3)**0.5)
...
for i in range(test_data_size):
    ...
    aposteriori_1 = normalization_factor_1 * np.exp(-0.5*np.dot((test_datapoint - mean_1),np.dot(cov_1_inv,(test_datapoint - mean_1).T)))
    aposteriori_2 = normalization_factor_2 * np.exp(-0.5*np.dot((test_datapoint - mean_2),np.dot(cov_2_inv,(test_datapoint - mean_2).T)))
    aposteriori_3 = normalization_factor_3 * np.exp(-0.5*np.dot((test_datapoint - mean_3),np.dot(cov_3_inv,(test_datapoint - mean_3).T)))
    aposteriori_1 = aposteriori_1.flatten()
    aposteriori_2 = aposteriori_2.flatten()
    aposteriori_3 = aposteriori_3.flatten()
